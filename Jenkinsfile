    ...
    stages {
        stage("build"){
            steps {
                script {
            sh """
               aws ecr get-login-password --region ap-south-1 | sudo docker login --username AWS --password-stdin 315073111691.dkr.ecr.ap-southeast-1.amazonaws.com/microbackend
